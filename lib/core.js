// devtrack core — logika bersama untuk server lokal (server.js) dan Vercel functions (api/*)
// Stateless terhadap penyimpanan: watchlist & settings hidup di localStorage browser.
...
// ---------- throttled fetch queue untuk pump.fun ----------
const pumpQueue = [];
let pumpActive = 0;
let pumpLastStart = 0;
...
function scoreDev(wallet, coins) {
  ...
  const flags = [];
  if (graduated > 0) flags.push({ good: true, text: `${graduated} token graduate ke DEX` });
  if (bestAth >= 1_000_000) flags.push({ good: true, text: `Pernah cetak token dengan ATH $${fmtNum(bestAth)}` });
  if (total >= 8 && gradRate < 0.05) {
    score -= 25;
    flags.push({ good: false, text: `Serial launcher: ${total} token, hampir tidak ada yang graduate` });
  }
  if (total > 20) {
    score -= 10;
    flags.push({ good: false, text: `Spam launch: ${total} token dibuat` });
  }
  if (total >= 3 && tractionRate === 0) flags.push({ good: false, text: 'Semua token mati tanpa traksi' });
  if (total === 1) flags.push({ good: null, text: 'Wallet baru: baru 1 kali launch (track record terbatas)' });
...
// ---------- Solana RPC (publik, atau Helius per-request) ----------
...
async function resolveAddress(addr, heliusKey) {
  ...
  } catch { /* bukan token pump.fun */ }
  if (await isMint(addr, heliusKey)) {
    const found = await findDeployer(addr, heliusKey);
    if (!found) {
      const err = new Error('Token ini terdeteksi tapi riwayatnya terlalu panjang untuk dilacak via RPC gratis.' +
        (heliusKey ? '' : ' Isi Helius API key di Pengaturan agar bisa menelusuri lebih dalam.'));
      err.status = 400;
      throw err;
    }
...
async function scanDeploys(wallet, heliusKey) {
  if (!heliusKey) {
    const err = new Error('Fitur ini butuh Helius API key (gratis) — isi di tab Pengaturan');
    err.status = 400;
    throw err;
  }
...
async function sendTelegram({ token, chatId }, html) {
  if (!token || !chatId) throw new Error('Telegram belum dikonfigurasi');
  ...
  if (!body.ok) {
    ...
    if (/blocked by the user/i.test(d)) throw new Error('Bot kamu diblokir / belum di-Start. Buka chat dengan bot di Telegram, tekan Start (atau Unblock), lalu tes lagi.');
    if (/chat not found/i.test(d)) throw new Error('Chat ID salah, atau kamu belum pernah menekan Start di chat bot. Cek chat ID via @userinfobot dan pastikan sudah Start.');
    if (/unauthorized/i.test(d)) throw new Error('Bot token salah — salin ulang token dari @BotFather (format 123456789:AAF...).');
    throw new Error(`Telegram: ${d || 'gagal kirim'}`);
  }
  return true;
}
...
function scanSignalHtml(modeLabel, filter, devs) {
  const lines = devs.slice(0, 10).map((d, i) =>
    ...
    `    skor ${d.score} · ${d.totalLaunches} launch · ${d.graduated} graduate (${pct(d.gradRate)}) · ATH ${usdShort(d.bestAthUsd)}\n` +
  ...
  return `\u{1F4E1} <b>SIGNAL SCAN — ${tgEsc(modeLabel)}</b>\n` +
    `Filter: min ${filter.minLaunch} launch, min winrate ${filter.minWinrate}%\n` +
    `${devs.length} dev lolos filter\n\n` + lines.join('\n\n');
}

function watchlistSignalHtml(item, coin, stats) {
  const label = item.label ? ` (${tgEsc(item.label)})` : '';
  const track = stats && stats.isCreator
    ? `${stats.launches}${stats.truncated ? '+' : ''} launch · ${stats.graduated} graduate · ATH ${usdShort(stats.bestAthUsd)}`
    : 'track record tidak tersedia';
  return `\u{1F514} <b>SIGNAL WATCHLIST — LAUNCH BARU</b>\n` +
    `Dev di watchlist kamu baru saja launch token!\n\n` +
